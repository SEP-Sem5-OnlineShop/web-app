    ...
    return (
        ...
        <div className="flex justify-between rounded-2xl overflow-hidden shadow-md bg-white h-full xs:h-24 sm:h-28 md:h-36 transform hover:scale-105 hover:shadow-lg transition ease-out duration-400 dark:bg-black" >
            ...
            <div className="mx-2 my-1 xs:mx-4 xs:my-2 flex flex-col justify-between items-start">
                <Link className="text-xs xs:text-sm sm:text-base md:text-lg text-secondary font-semibold dark:text-white" to={`/vendor_${vendor_id}/product_${product._id}`}>{ product.product_name }</Link>
                <span className="text-xs xs:text-sm sm:text-base md:text-lg text-secondary dark:text-white">{productStrings.available}: { stockproduct.stock }</span>
                <span className="text-xs xs:text-sm sm:text-base md:text-lg text-secondary dark:text-white">{productStrings.currency} { stockproduct.price }</span>
            </div>
            <div className="mr-1 my:4 sm:mx-4 flex flex-col justify-center items-end">
                <button className="rounded-xl shadow w-6 h-6 xs:w-8 xs:h-8 sm:w-10 sm:h-10 flex justify-center items-center bg-white transform hover:scale-110 hover:shadow-md transition ease-out duration-400 dark:bg-text" onClick={() => handleRemove(product._id)}>
